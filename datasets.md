...
| Property                | Value                                     |
| ----------------------- | ----------------------------------------- |
| **Source**              | Kaggle (`orig_irrigation.zip`)            |
| **Files**               | `irrigation_prediction.csv`               |
| **Rows**                | 10,000                                    |
| **Spatial resolution**  | Per crop type                             |
| **Temporal resolution** | Synthesized 30-min (no source timestamps) |

Static agronomic records (soil type, pH, moisture, temperature, humidity, rainfall, crop type, growth stage, irrigation need). No water-applied data, so `irrigated_next_flag` is all False.

### Kaggle IoT Sensor

| Property                | Value                                     |
| ----------------------- | ----------------------------------------- |
| **Source**              | Kaggle (`pi_iot.zip`)                     |
| **Files**               | `iotsensordata.csv`                       |
| **Rows**                | 100,000                                   |
| **Spatial resolution**  | Single plot                               |
| **Temporal resolution** | Synthesized 30-min (no source timestamps) |

100k sensor readings (soil moisture, temperature, humidity, rainfall, N/P/K) with ON/OFF pump status. No water-applied data, so `irrigated_next_flag` is all False.

### Kaggle Smart Agriculture

| Property                | Value                                     |
| ----------------------- | ----------------------------------------- |
| **Source**              | Kaggle (`smart_agri.zip`)                 |
| **Files**               | `cropdata_updated.csv`                    |
| **Rows**                | 16,411                                    |
| **Spatial resolution**  | Per crop type                             |
| **Temporal resolution** | Synthesized 30-min (no source timestamps) |

Crop moisture/temperature/humidity records with a binary irrigation result. No water-applied data, so `irrigated_next_flag` is all False.
...
| Kaggle Irrigation Prediction | 10,000  | per crop | synthesized       | 30 min    |
| Kaggle IoT Sensor            | 100,000 | 1        | synthesized       | 30 min    |
| Kaggle Smart Agriculture     | 16,411  | per crop | synthesized       | 30 min    |
